@if (config('livewire-tables.theme') === 'tailwind')
    <div class="flex rounded-md shadow-sm">
        <input
            ...
            type="text"
            class="block w-full border-slate-300 rounded-md shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 dark:bg-slate-800 dark:text-white dark:border-slate-600 @if (isset($columnSearch[$field]) && strlen($columnSearch[$field])) rounded-none rounded-l-md focus:ring-0 focus:border-slate-300 @else focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md @endif"
        />

        @if (isset($columnSearch[$field]) && strlen($columnSearch[$field]))
            <span wire:click="$set('columnSearch.{{ $field }}', null)" class="inline-flex items-center px-3 text-slate-500 border border-l-0 border-slate-300 cursor-pointer bg-slate-50 rounded-r-md sm:text-sm dark:bg-slate-700 dark:text-white dark:border-slate-600 dark:hover:bg-slate-600">
