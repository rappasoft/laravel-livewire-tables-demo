...
Column::make('Paid Revenue', 'orders_sum_total')
    ...
    ->format(fn($value) => '$' . number_format($value ?? 0, 2)),</code></pre>
        ...
        <div>
            <h3 class="text-lg font-semibold text-white mb-3 flex items-center">
                <span class="w-6 h-6 bg-pink-500/20 rounded flex items-center justify-center mr-2 text-pink-400 text-sm">4</span>
                Average Aggregate
            </h3>
            <p class="text-slate-400 mb-4">
...
Column::make('AOV', 'orders_avg_total')
    ...
    ->format(fn($value) => '$' . number_format($value ?? 0, 2)),</code></pre>
        ...
        <div>
            <h3 class="text-lg font-semibold text-white mb-3 flex items-center">
                <span class="w-6 h-6 bg-indigo-500/20 rounded flex items-center justify-center mr-2 text-indigo-400 text-sm">5</span>
                Min/Max Aggregates
            </h3>
            <p class="text-slate-400 mb-4">
...
Column::make('Highest Price', 'products_max_price')
    ...
    ->format(fn($value) => '$' . number_format($value ?? 0, 2)),</code></pre>
        ...
        <div>
            <h3 class="text-lg font-semibold text-white mb-3 flex items-center">
                <span class="w-6 h-6 bg-purple-500/20 rounded flex items-center justify-center mr-2 text-purple-400 text-sm">6</span>
        ...
        <div>
            <h3 class="text-lg font-semibold text-white mb-3 flex items-center">
                <span class="w-6 h-6 bg-emerald-500/20 rounded flex items-center justify-center mr-2 text-emerald-400 text-sm">7</span>
                Complete Example
            </h3>
            <p class="text-slate-400 mb-4">
                Here's a complete example showing various relationship aggregates:
            </p>
            <pre class="bg-slate-800/50 rounded-lg p-4 text-sm overflow-x-auto"><code class="text-slate-300">public function columns(): array
{
    return [
        ...
        Column::make('Completed', 'orders_count')
            ->counts(['orders' => fn($q) => $q->where('status', 'completed')])
            ->sortable(),
